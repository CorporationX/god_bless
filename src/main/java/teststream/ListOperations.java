...
public class ListOperations {
    ...
    public static long countStringsStartingWith(List<String> strings, char character) {
        ...
        return strings.stream().filter(string -> string.startsWith(String.valueOf(character))).count();
    }
    ...
    public static int findMinGreaterThan(List<Integer> numbers, int threshold) {
        Objects.requireNonNull(numbers, "List cannot be null.");
        return numbers.stream().filter(number -> number > threshold).mapToInt(Integer::intValue)
                .min().orElseThrow(() -> new NoSuchElementException("No value present"));
    }
