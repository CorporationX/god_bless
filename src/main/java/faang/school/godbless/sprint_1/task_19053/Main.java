...
public class Main {
    ...
    public static WeatherData getWeatherInformationByCityName(String cityName) {
        return WEATHER_DATA_CACHE.computeIfAbsent(cityName,
                (key) -> ExternalServiceWithWeatherData.getWeatherData(key));
    }

    public static void updateWeatherDataForCity(City city, WeatherData weatherData) {
        WEATHER_DATA_CACHE.replace(city.getName(), weatherData);
    }

    public static void deleteWeatherDataForCity(City city) {
