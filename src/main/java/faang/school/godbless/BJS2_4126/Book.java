...
public class Book {
    ...
    public static void addBook(Book book, String address, HashMap<Book, String> target) {
        target.put(book, address);
    }

    public static void removeBook(String title, String author, int year, HashMap<Book, String> target) {
        Book bookBeingDeleted = new Book(title, author, year);
        target.remove(bookBeingDeleted);
    }

    public static String findAddressByBook(String title, String author, int year, HashMap<Book, String> target) {
    ...
    public static void printAllBooks(HashMap<Book, String> target) {
        System.out.println("Список всех книг и их местонахождений:");
        for (HashMap.Entry<Book, String> entry : target.entrySet()) {
