...
public class NotificationManager {
    ...
    //TODO:12.06.2024 - неуверен за реализацю, хотел бы как нибудь максимально от этих if избавиться + хотел бы узнать уместно ли тут применить патерн Цепочка обязанностей?
                ...
                for (int j = 0; j < messageWord.length(); j++) {
                    ...
                    if (filter.test(substringMessageWord)) {
                        if (!mapperHandlers.containsKey(type)) {
                           throw new RuntimeException("Отказано в отправке");
                        } else {
