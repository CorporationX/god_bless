...
public class StreamsUtil {
    ...
    public static List<List<Integer>> findUniquePairs(List<Integer> numbers, int sum) {
        ...
        return numbers
                .stream()
                .filter(num -> num < sum)
                .flatMap(numOne -> numbers
                        .stream()
                        .filter(numTwo -> numOne + numTwo == sum)
                        .map(numTwo -> List.of(numOne, numTwo)
                                .stream()
                                .sorted()
                                .toList()))
                .distinct()
    ...
    public static List<String> filteredStringSortedStringByChar(List<String> strings, char checkChar) {
        ...
        return strings
                .stream()
                .filter(str -> str.charAt(0) == checkChar)
    ...
    public static List<List<String>> findPeopleNonFriends(Map<String, List<String>> personFriends) {
        ...
        return personFriends.entrySet()
                .stream()
                .flatMap(entryOne -> personFriends.entrySet()
                        .stream()
                        ...
                        .map((person) -> List.of(entryOne.getKey(), person.getKey())
                                .stream()
                                .sorted()
                                .toList()))
                .distinct()
    ...
    public static List<String> filterAndSortStringsByAlphabet(List<String> strings, char[] alphabet) {
        ...
        return strings
                ...
                .filter(str -> str.chars()
                        .allMatch(character -> containsCharacter(character, alphabet)))
                .sorted(Comparator.comparingInt(String::length))
                .toList();
    }
    ...
    public static List<Integer> findPalindromes(int start, int end) {
        return IntStream.range(start, end).boxed()
                .filter(num -> num / 10 != 0)
                .filter(StreamsUtil::checkNumberIsPalindromes)
                .toList();
    }
