...
public class Main {
    ...
    public static Integer findMax(List<Integer> integers) {
        return integers.stream().mapToInt(integer -> integer).max().orElse(0);
    }

    //Найти среднее значение чисел в списке;
    public static double findAverage(List<Integer> integers) {
        return integers.stream().mapToDouble(Integer::doubleValue).average().orElse(0.0);
    }
    ...
    public static long matchesWithCharachter(List<String> strings, char c) {
        return strings.stream().filter(str -> str.matches("^" + c + ".*")).count();
    }
    ...
    public static List<String> startsWithSubString(List<String> strings, String subString) {
        return strings.stream().filter(str -> str.startsWith(subString)).toList();
    }

    //Отсортировать список строк по длине;
    public static List<String> sortByLength(List<String> strings) {
        return strings.stream().sorted(Comparator.comparingInt(String::length))  //I wrote "(str1, str2) -> str1.length() - str2.length()", but one more time was offered a new solution, that you can see
                .toList();
    }
    ...
    public static Optional<Integer> findTheLeastListElement(List<Integer> ints, int i) {
        return ints.stream().filter(a -> a > i).min(Integer::compareTo);
    }
