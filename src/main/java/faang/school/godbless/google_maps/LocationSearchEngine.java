...
public class LocationSearchEngine {
    ...
    public List<Location>  filterLocations(List<Location> locationList, Predicate<Location> filter){
        return locationList.stream().filter(filter).collect(Collectors.toList());
    }

    public void processLocations(List<Location> locationList, Consumer<Location> consumer){
        locationList.stream().forEach(consumer);

    }

    public List<Double> calculateDistances(List<Location> locationList, Function<Location, Double> function){
        List<Double> distances = locationList.stream().map(function).collect(Collectors.toList());
