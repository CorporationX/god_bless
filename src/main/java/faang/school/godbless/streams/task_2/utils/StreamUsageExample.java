...
public class StreamUsageExample {
    public void printTermsForNumber(List<Integer> numbers, int number) {
        ...
        nums.stream()
                ...
                .forEach(num -> terms.add(List.of(num, number - num)));
    ...
    public Map<String, String> findPairsWithMutualFriends(Map<String, List<String>> friends) {
        ...
        friends.forEach((firstFriend, friendsOfFirstFriend) -> friends.entrySet().stream()
                .flatMap(entry -> entry.getValue().stream())
                .filter(friend -> !friendsOfFirstFriend.contains(friend) && !friend.equals(firstFriend))
                .forEach(friend -> friends.get(friend).stream()
                        .filter(fr -> friendsOfFirstFriend.contains(fr))
                        .forEach(name -> {
                            if (!pairs.containsKey(friend)) {
                                pairs.put(firstFriend, friend);
                            }
                        })));
        return pairs;
    }
