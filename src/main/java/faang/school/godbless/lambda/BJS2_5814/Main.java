package faang.school.godbless.lambda.BJS2_5814;

import java.util.function.Function;

public class Main {
    public static void main(String[] args) {
        System.out.println(triangleArea(5.0, 5.0, 9.0));
    ...
    private static double triangleArea(double a, double b, double c) throws IllegalArgumentException {
        if (a <= 0 || b <= 0 || c <= 0 || (a >= b + c) || (b >= a + c) || (c >= a + b)) {
            throw new IllegalArgumentException("Стороны треугольника неверны по правилу \"Неравенства треугольника\"");
        }
