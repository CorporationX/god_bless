...
public class StreamApiTraining {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("isMatch: " + isMatch(words));
    ...
    public static int countStringsStartsWith(@NonNull List<String> words, char s) {
        return words.stream().filter(word -> word.startsWith(String.valueOf(s))).mapToInt(o -> 1).sum();
    }
    ...
    public static List<String> sortWordsByLength(@NonNull List<String> words) {
        return words.stream().sorted((o1, o2) -> o1.length() - o2.length()).collect(Collectors.toList());
    }

    public static boolean isMatch(@NonNull List<String> words) {
        return words.stream().allMatch(word -> !word.isBlank());
    }

    public static int findMinFromBiggerNums(@NonNull List<Integer> numbers, int source) {
        return numbers.stream().filter(num -> num > source).mapToInt(num -> num).min().getAsInt();
    }

    public static List<Integer> mapStringsByLength(@NonNull List<String> words) {
        return words.stream().mapToInt(word -> word.length()).boxed().toList();
    }
}
