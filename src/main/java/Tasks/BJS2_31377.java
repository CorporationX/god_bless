...
public class BJS2_31377
{
	public void run()
	{
		...
		System.out.println("Коллекции Map состоящий из ключа возраста пользоватей и списка пользователей");
...
class User
{
	...
	public static Map<Integer, List<User>> groupUser(List<User> users)
	{
		...
		for (User user : users)
		{
			...
			if (!groupedUsersByAge.containsKey(age)) {
				groupedUsersByAge.put(age, new ArrayList<>());
			}
