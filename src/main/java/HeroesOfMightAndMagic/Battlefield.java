...
public class Battlefield {
    ...
    private void damageExchange(Creature creature1, Creature creature2, Hero hero1, Hero hero2) {
        ...
        System.out.printf("%s нанес %s %d урона%n", creature1.getName(), creature2.getName(), damageTo);

        if(creature2.getHealth() <= 0){
            hero2.removeCreature(creature2, 1);
            System.out.printf("Существо %s погибло. У героя %s осталось %d существ%n", creature2.getName(), hero2.getName(), hero2.getArmy().size());
        }else{
            damageTo = creature2.getDamage() * (1 - creature1.getArmor()/100);
            creature1.setHealth(creature1.getHealth() - damageTo);
            System.out.printf("%s нанес %s %d урона%n", creature2.getName(), creature1.getName(), damageTo);
            if(creature1.getHealth() <= 0){
                hero1.removeCreature(creature1, 1);
                System.out.printf("Существо %s погибло. У героя %s осталось %d существ%n", creature1.getName(), hero1.getName(), hero1.getArmy().size());
            }
        }
    }
