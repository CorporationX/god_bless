...
public class StreamLogic {
    ...
    public static double averageNumber(List<Integer> numbers) {
        double sum = numbers.stream().reduce(0, (area, number) -> area + number);
        return sum / numbers.size();
    }

    public static long countLinesStartingChar(List<String> lines, char symbol) {
        return lines.stream().filter(line -> symbol == line.charAt(0)).toList().size();
    }

    public static List<String> stringsWithSubstring(List<String> lines, String substring) {
        return lines.stream().filter(line -> line.contains(substring)).toList();
    }
    ...
    public static boolean checkingCondition(List<Integer> numbers, Predicate<Integer> predicate) {
        return numbers.stream().allMatch(predicate::test);
    }

    public static int biggerThanSomeNumber(List<Integer> numbers, int entryNumber) {
        return numbers.stream().filter(number -> number > entryNumber).min(Integer::compareTo)
    ...
    public static List<Integer> linesInNumbers(List<String> lines) {
        return lines.stream().map(line -> Integer.valueOf(line.length())).toList();
    }
}
