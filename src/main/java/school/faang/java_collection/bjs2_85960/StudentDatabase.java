...
public class StudentDatabase {
    ...
    public void removeStudent(Student student) {
        ...
        if (grades != null) {
            ...
            System.out.println("Исключен студент: " + student.getName());
        } else {
            System.out.println("Студента '" + student.getName() + "' не существует");
        }
    }
    ...
    void printAllStudentsWithGrades() {
        for (Map.Entry<Student, Map<Subject, Integer>> entry : studentSubjects.entrySet()) {
            ...
            for (Map.Entry<Subject, Integer> entry2 : entry.getValue().entrySet()) {
                Subject currentSubject = entry2.getKey();
                System.out.println(" - " + currentSubject.getName() + ": " + entry2.getValue());
            }
        }
    }
    ...
    public void printAllSubjectsWithStudents() {
        for (Map.Entry<Subject, List<Student>> entry : subjectStudents.entrySet()) {
            ...
            System.out.println("\nПредмет '" + subject.getName() + "' изучают:");
