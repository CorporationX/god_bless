...
public class Main {
    public static void main(String[] args) {
        ...
        getMaxNumber(numbers).ifPresentOrElse(
                obj -> System.out.printf("2. Максимальное число в списке: %d%n", obj),
                () -> System.out.println("Список пуст\n"));
        evaluateArithmeticMean(numbers).ifPresentOrElse(
                obj -> System.out.printf("3. Среднее арифметическое списка: %.3f%n", obj),
                () -> System.out.println("Список пуст\n"));
        System.out.printf("4. Список соответствует условию: %b%n", isMatch(numbers, number -> number > 20));
        getNumberGreaterThan(numbers, 50).ifPresentOrElse(
                obj -> System.out.printf("5. Минимальное число после заданного: %d%n", obj),
                () -> System.out.println("Список пуст\n"));
        ...
        System.out.printf("6. Количество строк, начинающихся с заданного символа: %d%n",
                countStringsStartingWith(strings, 'в'));
        System.out.println("7. Строки, включающие с заданную строку:");
        getStringsContainsSubstring(strings, "список").forEach(System.out::println);
        ...
        convertToLength(strings).forEach(System.out::println);
    }
    ...
    public static OptionalDouble evaluateArithmeticMean(List<Integer> numbers) {
        return numbers.stream()
                .mapToInt(Integer::intValue)
                .average();
    }
    ...
    public static Optional<Integer> getNumberGreaterThan(List<Integer> numbers, int bound) {
        return numbers.stream()
