...
public class Main {
    private static final String emailMessage = """
            Похоже, кто-то пытался войти в вашу учётную запись Discord из нового местоположения.
            Если это вы, перейдите по ссылке, чтобы разрешить вход.""";
    private static final String galaxyMessage = """
             Привет из Млечного Пути! Мы — разумные существа (но это не точно) с планеты Земля, изучаем Вселенную
             и надеемся, что не одиноки. Если вы получаете это сообщение, ответьте любым возможным способом. 
             Мир и дружба!""";
    private static final String  pushMessage = "Пользователь Evan поставил like вашему сообщению";
    private static final String faxMessage = "Добрый день! Высылаю вам проект договора.";
    ...
    public static void main(String[] args) {
        ...
        notificationManager.registerHandler(NotificationType.SMS, (notification -> System.out.println(
                "Реализуем логику отправки смс и прикрепляем текст: " + notification.getMessage())));
        ...
        notificationManager.registerHandler(NotificationType.FAX, null);
        Notification notification4 = new Notification(NotificationType.FAX, faxMessage);
        notificationManager.sendNotification(notification4);
