...
public class UserActionAnalyzer {
    ...
    public static List<String> topActiveUsers(List<UserAction> actions) {
        return actions.stream()
                ...
                .limit(10)
                .map(entry -> actions.stream().filter(action -> action.getId() == entry.getKey()).findFirst())
                .filter(Optional::isPresent)
                .map(optionalAction -> optionalAction.get().getName())
                .toList();
    }
    ...
    public static Map<String, Double> actionTypePercentages(List<UserAction> actions) {
        ...
        return actions.stream()
                ...
                .entrySet().stream()
                .map(entry -> Map.entry(entry.getKey().name(), (entry.getValue() * 100.0) / totalActions))
