...
public class AnalyzerTesting {
    public static void main(String[] args) {
        ...
        List<String> top10Users = UserActionAnalyzer.getTopNUsers(actions, 3);
        System.out.println("Топ-3 активных пользователя: " + top10Users);

        List<String> top5Hashtags = UserActionAnalyzer.mostPopularHashtags(actions, 3);
        System.out.println("Топ-3 популярных хэштега: " + top5Hashtags);

        List<String> top3Commenters = UserActionAnalyzer.mostActiveUsers(actions, 3);
        System.out.println("Топ-3 комментаторов за последний месяц: " + top3Commenters);

        Map<ActionType, Double> actionPercentages = UserActionAnalyzer.calculatePercent(actions);
