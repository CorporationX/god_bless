...
public class PostService {
    ...
    public void viewPost(Post post) {
        log.info("Пост {} с заголовком '{}' написал {}. Содержание поста: {}.",
                post.getId(), post.getTitle(), post.getAuthor(), post.getContent());
    }

    public void viewComments(Post post) {
        StringBuilder commentsString = new StringBuilder();
        commentsString.append("Комменты поста ").append(post.getId()).append(": ");
        post.getComments().forEach(comment -> commentsString.append(comment.text()).append("\n"));
        log.info(commentsString.toString());
    }
