package school.faang.task223;

import school.faang.task223.user.UserAction;
import school.faang.task223.user.UserActionAnalyzer;
...
public class Main {
    public static void main(String[] args) {
        List<UserAction> actions = Arrays.asList(
                new UserAction(1, "Alice", "post", LocalDate.of(2024, 9, 1), "Check out this amazing #newfeature!56"),
        ...
        List<String> topUsers = UserActionAnalyzer.returnTop10Users(actions);
        topUsers.forEach(System.out::println);

        List<String> topHashtags = UserActionAnalyzer.topHashtags(actions);
        topHashtags.forEach(System.out::println);

        List<String> topCommenters = UserActionAnalyzer.topCommenter(actions);
        topCommenters.forEach(System.out::println);

        Map<String, Double> actionTypePercen = UserActionAnalyzer.actionTypePercentages(actions);
        actionTypePercen.forEach((s, aDouble) -> System.out.println(s + " " + aDouble + "%"));
    }
}
