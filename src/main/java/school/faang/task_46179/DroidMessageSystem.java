...
public class DroidMessageSystem {
    ...
    public void sendMessage(int key, String message, Droid sender, Droid recipient) {
        String encryptedMessage = messageEncryptor.encrypt(key, message);
        log.info("{} sent an encrypted message: {}", sender.getName(), encryptedMessage);

        String decryptedMessage = receiveMessage(key, encryptedMessage);
        log.info("{} received an encrypted message: {}", recipient.getName(), decryptedMessage);
    }
