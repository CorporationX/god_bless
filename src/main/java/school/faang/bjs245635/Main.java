...
public class Main {
    public static void main(String[] args) {
        ...
        inventoryManager.updateItem(bubba, item -> item.getName().equals("Bone"), item -> new Item(item.getName(), item.getValue() * 5));
        inventoryManager.updateItem(bubba, item -> item.getName().equals("Ball"), item -> new Item(item.getName(), item.getValue() * 5));
