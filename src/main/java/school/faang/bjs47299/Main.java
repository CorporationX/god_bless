...
public class Main {
    public static void main(String[] args) {
        ...
        List<User> topUsers = AnalyticsService.topTen(actions);
        System.out.println("Топ-10 активных пользователей: " + topUsers);

        var topHashtags = AnalyticsService.topTopics(actions);
        System.out.println(topHashtags);

        var topMonthComments = AnalyticsService.topUsersLastMonth(actions);
        System.out.println(topMonthComments);

        var actionTypeFrequences = AnalyticsService.actionTypeRelation(actions);
        System.out.println(actionTypeFrequences);
    }
}
