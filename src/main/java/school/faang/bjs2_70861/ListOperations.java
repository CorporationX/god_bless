...
public class ListOperations {
    ...
    public static double findAverage(List<Integer> numbers) {
        return Optional.of((double) (numbers.stream().mapToInt(number -> number).sum() / numbers.size())).orElse(-1.0);
    }

    public static long countStringsStartingWith(List<String> lines, char character) {
        return lines.stream().filter(line -> line.charAt(0) == character).count();
    }
    ...
    public static List<String> sortByLength(List<String> lines) {
        return lines.stream().sorted((line1, line2) -> line1.length() >= line2.length() ? 1 : -1).toList();
    }
    ...
    public static int findMinGreaterThan(List<Integer> numbers, int min) {
        return numbers.stream().filter(num -> num > min).reduce(Integer::min).orElse(-1);
    }
