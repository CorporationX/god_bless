...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        NotificationManager amazonMessageManager = new NotificationManager(new DumbModerator(), new AmazonCorrector())
                .registerHandler(NotificationType.EMAIL, notification -> System.out.printf((messageTemplate) + "%n", notification.type(), notification.message()))
                .registerHandler(NotificationType.SMS, notification -> System.out.printf((messageTemplate) + "%n", notification.type(), notification.message()))
                .registerHandler(NotificationType.PUSH, notification -> System.out.printf((messageTemplate) + "%n", notification.type(), notification.message()));
        ...
        try {
            twitterMessageManager.sendNotification(new Notification(NotificationType.PUSH, "send push with bad word 1 to you"));
        } catch (IllegalMessageException e) {
            log.warn(e.getMessage());
        }

        try {
            amazonMessageManager.sendNotification(new Notification(NotificationType.EMAIL, "send email with bad word 2 to you"));
