...
public class SpellEvent {
    ...
    public SpellEvent(String eventType, String action) {
        this.id = ++counter;
        if (eventType == null) {
            log.error("EventType cannot be null. " +
                    "Please provide a valid EventType instance to initialize the SpellEvent.");
        ...
        if (action == null || action.trim().isEmpty()) {
            log.error("Action cannot be null or empty. " +
                    "Please provide a valid action string to initialize the SpellEvent.");
