...
public record CityWorker(City city, List<Monster> monsters) implements Runnable {
    ...
    @Override
    public void run() {
        int bSide = city.getLocation().getY();
        int aSide = city.getLocation().getX();

        double cSide = Math.sqrt(Math.pow(bSide, 2) + Math.pow(aSide, 2));
        System.out.printf("The path from Witcher to city is %.2f km\n", cSide);

        double minPathToMonster = monsters.stream()
                .map((monster) -> Math.sqrt(Math.pow(Math.abs(monster.getLocation().getX() - city.getLocation().getX()), 2)
                        + Math.pow(Math.abs(monster.getLocation().getY() - city.getLocation().getY()), 2)))
