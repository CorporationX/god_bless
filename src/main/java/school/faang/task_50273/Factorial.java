...
public class Factorial {
    ...
    public static int factorialInt(int n) {
        if (n < 0 || n > MAX_INT_FACTORIAL) {
            throw new IllegalArgumentException("Your number must be greater than -1 and less than "
                    + MAX_INT_FACTORIAL + 1);
        }
    ...
    public static BigInteger factorialBig(int n) {
        if (n < 0) {
            throw new IllegalArgumentException("Your number must be greater than -1");
