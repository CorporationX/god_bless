...
public class ListOperations {
    ...
    public static long countStringsStartingWith(List<String> strings, char letter) {
        return strings.stream().filter(s -> !s.isEmpty() && s.charAt(0) == letter).count();
    }

    public static List<String> filterStringsContainingSubstring(List<String> strings,
                                                          String particle) {
        return strings.stream().filter(s -> s.contains(particle)).toList();
    }
    ...
    public static int findMinGreaterThan(List<Integer> numbers, int numb) {
        return numbers.stream().filter(n -> n > numb).findFirst()
