...
public class Main {
    ...
    private static void removeBook(String author, String title, int year) {
        Map<Book, String> clone = new HashMap<>(mappa);
        for (Map.Entry<Book, String> entry : clone.entrySet()) {
            if(checkBook(author, title, year, entry.getKey())) {mappa.remove(entry.getKey());}
        }
    }
    ...
    private static String getLine(String author, String title, int year) {
        for (Map.Entry<Book, String> entry : mappa.entrySet()) {
            if(checkBook(author, title, year, entry.getKey())) return entry.getValue();
        }
        return null;
    }
