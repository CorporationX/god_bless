...
class UserActionAnalyzerTest {
    ...
    void topActiveUsers() {
        ...
        assertEquals(List.of("Alice", "Bob", "Charlie", "Dave", "Eve"), topUsers);
    }

    @Test
    void topPopularHashtags() {
        List<String> topHashtags = UserActionAnalyzer.topPopularHashtags(ACTIONS);
        assertEquals(List.of("#update", "#feature", "#Java", "#conference"), topHashtags);
    }

    @Test
    void topCommentersLastMonth() {
        List<String> topCommenters = UserActionAnalyzer.topCommentersLastMonth(ACTIONS);
        assertEquals(List.of("Charlie", "Eve", "Dave"), topCommenters);
    }

    @Test
    void actionTypePercentages() {
        Map<String, Double> actionPercentages = UserActionAnalyzer.actionTypePercentages(ACTIONS);
        assertEquals(Map.of("LIKE", 26.53061224489796, "POST", 26.53061224489796, "SHARE", 20.408163265306122, "COMMENT", 26.53061224489796), actionPercentages);
    }
}