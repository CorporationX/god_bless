...
class UserActionAnalyzerTest {
    ...
    void actionTypePercentagesTest() {
        Map<ActionType, Double> expected = Map.of(
                ...
                ActionType.SHARE, 20.0,
                ActionType.COMMENT, 26.0
        );
        Map<ActionType, Double> result = UserActionAnalyzer.actionTypePercentages(createTestUserActions());
        assertEquals(expected, result);
    }

    private List<UserAction> createTestUserActions() {
